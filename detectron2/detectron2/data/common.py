...
class AspectRatioAndDatasetGroupedDataset(data.IterableDataset):
    """
    Batch data that have similar aspect ratio, and from the same dataset together.
    In this implementation, images whose aspect ratio < (or >) 1 will
    be batched together.
    This improves training speed because the images then need less padding
    to form a batch.
    ...
    def __init__(self, dataset, batch_size, dataset_names):
        """
        Args:
            dataset: an iterable. Each element must be a dict with keys
                "width" and "height", which will be used to batch data.
    ...
    def __iter__(self):
        for d in self.dataset:
            ...
            aspr_id = 0 if w > h else 1
            dataset_id = self.dataset_name_to_id[d["dataset"]] 
            bucket_id = aspr_id * self.num_datasets + dataset_id
            bucket = self._buckets[bucket_id]
            bucket.append(d)
            if len(bucket) == self.batch_size:
                data = bucket[:]
